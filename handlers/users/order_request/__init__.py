from .create_request import dp
from .operation_type import dp
from .type_of_card_if_cash_in import dp

from .how_much_recive_message_handler import dp
from .how_much_give_message_handler import dp

from .temp_sum_message_handler import dp

from .currency__how_much import dp
from .currency__how_much__recive import dp
from .currency__how_much__give import dp
from .plus_or_minus_sum import dp

# from .adding_sum_message_handler import dp
# from .adding_sum_currency import dp


from .final_step_ordering import dp
from .comment import dp
from .permit import dp


__all__ = ['dp']